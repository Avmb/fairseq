...
class RND(nn.Module):
    def __init__(self, args, hidden_dim):
        ...
        self.rnd_base_n_hidden_layers = getattr(args, 'rnd_base_n_hidden_layers', 1)
        self.rnd_student_n_residual_blocks = getattr(args, 'rnd_student_n_residual_blocks', 1)
        self.rnd_student_residual_inner_dim = getattr(args, 'rnd_student_residual_inner_dim', 2048)
        ...
        for layer_id in range(self.rnd_base_n_hidden_layers):
            ...
            self.student_layers.append(nn.LeakyReLU())
        self.base_hidden = nn.Sequential(*self.base_layers)
        self.student_hidden = nn.Sequential(*self.student_layers)
        for param in self.base_hidden.parameters():
            param.requires_grad = False
        
        self.res_blocks = []
        for res_block_id in range(self.rnd_student_n_residual_blocks):
            res_block = nn.Sequential(nn.LayerNorm(self.hidden_dim, elementwise_affine=False),
                                       nn.Linear(self.hidden_dim, self.rnd_student_residual_inner_dim, bias=False),
                                       nn.LeakyReLU(),
                                       nn.Linear(self.rnd_student_residual_inner_dim, self.hidden_dim, bias=False))
    ...
    def forward(self, x):
        base = self.base_hidden(x)
        student = self.student_hidden(x)
        for res_block_id in range(self.rnd_student_n_residual_blocks):
            student = student + self.res_blocks[res_block_id](student)
        base = self.base_proj(base)
        student = self.student_proj(student)
        diff = base - student
        msq = (diff * diff).mean(dim=-1)
        return msq

    

class GRURNDDecoder(FairseqIncrementalDecoder):
    """
    GRU decoder with Random Network Distillation
    ...
    def __init__(
        ...
        # RND
        self.rnd_scale_state = torch.scalar_tensor(getattr(args, 'rnd_scale_state', -1.0))
        self.rnd_scale_output = torch.scalar_tensor(getattr(args, 'rnd_scale_output', -1.0))
        #
        ...
        # RND
        self.rnd_gain_state = nn.Parameter(torch.scalar_tensor(1.0))
        self.rnd_gain_output = nn.Parameter(torch.scalar_tensor(1.0))
        #
    ...
    def extract_features(
        ...
        for j in range(seqlen):
            ...
            for i, rnn in enumerate(self.layers):
                ...
                prev_hiddens[i] = rnd_project(hidden, rnd_err, self.rnd_scale_state, self.rnd_gain_state)
...
def rnd_project(x, rnd_err, scale, gain):
    c = gain * torch.exp(scale * rnd_err)
    #c = torch.zeros_like(c)
    rv = x * c.unsqueeze(-1)
    #print(rnd_err.mean(), scale, gain.data, c.mean())
    return rv

@register_model("transformer_gru_rnd")
class TransformerGRURNDModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--rnd-base-n-hidden-layers', type=int, metavar='N',
                            help='RND model: number of base layers')
        parser.add_argument('--rnd-student-n-residual-blocks', type=int, metavar='N',
                            help='RND model: number of residual blocks in a student')
        parser.add_argument('--rnd-student-residual-inner-dim', type=int, metavar='N',
                            help='RND model: inner dimension of student residual blocks')
        parser.add_argument('--rnd-scale-state', type=float, metavar='D',
...
@register_model_architecture("transformer_gru_rnd", "transformer_gru_rnd")
...
def transformer_gru_iwslt_de_en(args):
    ...
    args.decoder_layers = getattr(args, "decoder_layers", 2)
    # RND
    args.rnd_student_residual_inner_dim = getattr(args, 'rnd_student_residual_inner_dim', 1024)
    #
    base_architecture(args)